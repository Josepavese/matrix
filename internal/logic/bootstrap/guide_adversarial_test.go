...
func TestBuildGuideNamesWhatIsMissing(t *testing.T) {
	empty := strings.Join(BuildGuide(GuideInput{MatrixHTTPAddr: "127.0.0.1:9091", SystemConfigured: false, TelegramEnabled: false, TelegramConfigured: false, ActiveAgents: nil}), "\n")
	...
	halfConfigured := strings.Join(BuildGuide(GuideInput{MatrixHTTPAddr: "127.0.0.1:9091", SystemConfigured: true, TelegramEnabled: true, TelegramConfigured: false, ActiveAgents: []string{"codex"}}), "\n")
	...
	done := strings.Join(BuildGuide(GuideInput{MatrixHTTPAddr: "127.0.0.1:9091", SystemConfigured: true, TelegramEnabled: true, TelegramConfigured: true, ActiveAgents: []string{"codex"}}), "\n")
...
func TestTheGuideNamesTheAddressTheRuntimeWillBind(t *testing.T) {
	moved := strings.Join(BuildGuide(GuideInput{MatrixHTTPAddr: "127.0.0.1:9291", SystemConfigured: true, ActiveAgents: []string{"codex"}}), "\n")
	...
	fallback := strings.Join(BuildGuide(GuideInput{SystemConfigured: true, ActiveAgents: []string{"codex"}}), "\n")
