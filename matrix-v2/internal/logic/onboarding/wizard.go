...
// AgentEntry represents an agent available for selection during onboarding.
type AgentEntry struct {
	ID          string   `json:"id"`
	Name        string   `json:"name"`
	Description string   `json:"description"`
	DistTypes   []string `json:"dist_types"`
...
type WizardDependencies struct {
	...
	RegClient *agentmgr.RegistryClient
	FS        middleware.FS
...
func NewWizard(deps WizardDependencies) *Wizard {
	w := &Wizard{
		...
		regClient: deps.RegClient,
		fs:        deps.FS,
		net:       deps.Net,
	}
	// Fallback: derive registry client from installer if not provided directly
	if w.regClient == nil && w.installer != nil {
		w.regClient = w.installer.RegistryClient()
	}
	w.handlers = newAuthHandlerRegistry(w)
	return w
}

// fetchAgentList retrieves available agents from the ACP registry (cached)
// and marks which ones are already installed locally.
// Falls back to installed-only if the registry is unreachable.
func (w *Wizard) fetchAgentList() ([]AgentEntry, error) {
	...
	index, err := w.regClient.FetchIndexCached(ctx)
	if err != nil {
		slog.Warn("registry fetch failed, using installed agents only", "error", err)
		return w.installedAgents(), nil
	}

	installedIDs := w.installedIDSet()
	entries := make([]AgentEntry, 0, len(index.Agents))
	for _, a := range index.Agents {
		entries = append(entries, AgentEntry{
			ID:          a.ID,
			Name:        a.Name,
			Description: a.Description,
			DistTypes:   a.DistTypes(),
			Installed:   installedIDs[a.ID],
		})
	}
	return entries, nil
}
