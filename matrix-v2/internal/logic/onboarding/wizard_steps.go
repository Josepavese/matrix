...
// step2: Agent selection (dynamic from ACP registry)
...
func (w *Wizard) step2Handle(_ *Wizard, state *WizardState, input string) (string, error) {
	...
	if !selected.Installed && w.installer != nil {
		installMsg := fmt.Sprintf(w.localizer.GetString(state.Language, "agent_installing"), selected.Name)
		slog.Info("auto-installing agent from wizard", "agent", selected.ID)
		if err := w.installer.Install(context.Background(), selected.ID); err != nil {
