...
- session lifecycle methods including `session/list`, `session/load`, stable `session/resume`, stable `session/close`, `session/cancel`, explicit draft `session/fork`, and stable `session/delete`
- stable session configuration through `configOptions`, `config_option_update`, and `session/set_config_option`
- stable `$/cancel_request`, stable `logout`, and typed non-default draft surfaces for model/provider experiments
- stable lifecycle `additionalDirectories` plus explicitly retained draft response metadata used for protocol audits
...
- `Client`
- `ClientAPI`
- `ExperimentalClientAPI` for explicitly non-stable operations
- `Transport`
- `RequestHandler`
- `SessionObserver`
- `InitializeRequest`
...
- `session/set_config_option` is implemented as a stable request and decodes the complete returned `configOptions` state; Matrix uses stable modes only when config options are absent
...
- `authenticate` always uses the stable request shape (`methodId` only); retired credential payloads are not representable by the API
...
- optional stable capability flags accept the ACP object-presence shape such as `"close": {}`; retired boolean aliases are rejected
